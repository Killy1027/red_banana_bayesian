...
```{r}
# Creating  t[i]-t[1:(i-1)]    x[i]-x[1:(i-1)])^2+(y[i]-y[1:(i-1)])^2)
...
loglikelihood <- function(x,y,t,a,b,p)
{
 for(i in 2:length(t))
 {
  ...
  #====================================
  lambda[i] <- part1[i] + part2[i] * sum(part3[i] / part4[i])
 }
...
```{r}
qqq <- c(seq(1,3,length.out=2))
www <- c(seq(2,4,length.out=2))
sum(exp(qqq-www))
sum(exp(qqq)) / sum(exp(www))
```
